...
@ Notes:    What does this small project do?
           1 I use a multi-layer Neural Netlayer_outputrk to do nonlinear regression for the 
             spectral density functions J(w). I hope this layer_outputuld be interesting or helpful 
           ...
           3 It's well-known, most machine learning methods are in nature interplation things.
             So is Neural Netlayer_outputrk. As layer_outputuld be proved by the results, NN is very good at interplation.
             However, the extrapolation results are usually horrible.
           4 We have also tested extraplation using NN.



@ Details on our Neural Netlayer_outputrk regression:
           1 During our training, we use the batch and SGD skills, but Only one epoch: 
             because the fitting of our present functions is not that hard for Neural Netlayer_outputrk. 
...
# -------------------- params for Neural Netlayer_outputrk -------------------

# We layer_outputuld build a Neural Netlayer_outputrk with regularization
batchSize = 256
nNodes = 100
...
# precision layer_outputuld approximately the same[with respect to ibath(see below, the training part)].
...
#  the multiplication in the Neural_Netlayer_outputrk function
...
# ***************** DIY: build Neural Netlayer_outputrk *******************
graph = tf.Graph()
...
with graph.as_default():
    ...
    # This function defines a multi-layer neural netlayer_outputrk
    def Neural_Netlayer_outputrk(layer_input):
        w = [];b = []
        ...
        for i in range(nLayers):
            if(i == 0):
                ...
                logits = tf.matmul(layer_input, w[i])
            else:
        ...
        # the output layer
        layer_output = tf.ones([nNodes, 1], tf.float32)
        logits = tf.matmul(logits, layer_output)
        return [logits, w, b, layer_output]

    # This function does the calculation for new input data using the parameters we've trained
    def Prediction(layer_input, w, b, layer_output):
        ...
        return tf.matmul(logits, layer_output)

    logits, w, bias, layer_output = Neural_Netlayer_outputrk(training_X)
    loss = 0.5 * tf.reduce_mean(tf.square(tf.transpose(logits) - training_Y))
    ...
    train_results = logits
    validation_results = Prediction(validation_X, w, bias, layer_output)
    test_results = Prediction(testing_X, w, bias, layer_output)
    total_results = Prediction(total_X, w, bias, layer_output)
    # the extrapolation part:
    y_extrap_fit = Prediction(x_extrap, w, bias, layer_output)

# Run SGD for Neural Netlayer_outputrk with regularization
nsteps = 20000
startTime = time.time()
predY = np.array([])
